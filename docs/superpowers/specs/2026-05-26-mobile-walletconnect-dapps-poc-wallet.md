# Mobile WalletConnect dApps — POC (wallet-side implementation)

Single-PR scope for the wallet-side WalletConnect POC. Builds on the full feature designed in [2026-05-11-mobile-walletconnect-dapps-design.md](./2026-05-11-mobile-walletconnect-dapps-design.md) and ticketed in [2026-05-12-mobile-walletconnect-dapps-tickets.md](./2026-05-12-mobile-walletconnect-dapps-tickets.md). Supersedes the staged plan in [2026-05-21-mobile-walletconnect-dapps-poc-design.md](./2026-05-21-mobile-walletconnect-dapps-poc-design.md) — the Signer refactor (Part 1) and the Send-flow tx-preview prerequisites have landed on `dev`.
...
Both prerequisites are merged before this PR starts — assume the infrastructure is available.

- **Signer refactor** — merged via [safe-global/safe-wallet-monorepo#7912](https://github.com/safe-global/safe-wallet-monorepo/pull/7912). `apps/mobile/src/features/WalletConnect/{appKit.ts,components,context,hooks,utils}` now lives under `Signer/`, with `shared/{compat,projectId,metadata,mmkvStorageAdapter}.ts` extracted for reuse by `Wallet/`. This PR imports from `shared/` rather than re-implementing those primitives.
- **Send-flow refactor** (`danield/mobile-tx-preview`) — merged to `dev`. Provides `draftTxSlice`, `synthesizeDraftTxDetails`, and the `prebuiltSafeTx` signer param that the dApp transaction flow consumes verbatim.

Branch off `dev`.

## 1. Delivery shape

Single PR adding `apps/mobile/src/features/WalletConnect/Wallet/`. Implements pairing, session management, request handling, and the RPC method router end-to-end. Reviewed as one piece because the parts are tightly coupled and demoing the POC requires all of them.
...
- **RPC method router (Ticket 8, full).** Read-only proxy via ethers v6 `JsonRpcProvider` for the chain in `params.chainId`; allow-list as specified. `eth_accounts`/`eth_chainId`/`net_version` answered locally. `wallet_switchEthereumChain` → dispatch `switchActiveChain` + `updateSession`, respond `null`; non-deployed chain → `4901` (aligns with web). `wallet_getCapabilities` → `{atomicBatch: {supported: true}}` per approved chain. `wallet_getCallsStatus` → local Safe-tx status lookup. `wallet_showCallsStatus` → navigate to queue/tx-detail screen. Signing methods + `safe_setSettings` → `UNSUPPORTED_METHOD` without UI. `session_authenticate` event → `rejectSessionAuthenticate(UNSUPPORTED_METHOD)`. Unknown method → `UNSUPPORTED_METHOD`; cross-namespace method → `UNAUTHORIZED_METHOD`.
...
- **All Jest unit + integration tests** for the new code. This PR ships with no new test files. Existing tests must still pass.
...
├── Signer/                  # on dev via PR #7912
├── shared/                  # on dev via PR #7912
└── Wallet/                  # new in this PR
...
- **`shared/{compat,projectId,metadata,mmkvStorageAdapter}.ts`.** On `dev` via PR #7912.
- **`draftTxSlice`, `synthesizeDraftTxDetails`, `prebuiltSafeTx`.** On `dev` via the `danield/mobile-tx-preview` merge.

### Known risks to spike during implementation
...
- Not a refactor target for the existing AppKit / Signer code beyond what the prerequisite PR #7912 already covers.
...
## 7. Open items to resolve at start
