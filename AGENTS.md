...
## Transaction Flow System (Web)

The transaction flow system handles all user interactions for creating, signing, and executing Safe transactions. See `apps/web/docs/tx-flow-refactoring-plan.md` for detailed architecture documentation.

### Key Components

- **TxFlow** – Entry point wrapping all providers
- **SafeTxProvider** – Builds SafeTransaction from input
- **TxFlowProvider** – Central state management (navigation, execution state, form state)
- **SlotProvider** – Dynamic action registration system
- **ComboSubmit** – Smart action router with dropdown for multiple options

### Slot System

Actions (Sign, Execute, Batch, etc.) register dynamically based on conditions:
...
```
src/components/tx-flow/
├── actions/           # Sign, Execute, Propose, Batching, etc.
├── slots/             # Slot system (SlotProvider, withSlot)
├── features/          # TxNote, SignerSelect, BalanceChanges
├── common/            # TxCard, TxLayout
└── flows/             # 31 transaction type flows

src/components/tx/shared/
├── hooks.ts           # useTxActions, useValidateNonce, etc.
├── types.ts           # SignOrExecuteProps, SubmitCallback
└── errors/            # Error components
```

### When Making Changes

- Use `TxFlowContext` for state (never duplicate with local state)
- Register new actions via `withSlot()` HOC
- Shared utilities go in `tx/shared/`
- Follow the action selection decision tree in the architecture docs
