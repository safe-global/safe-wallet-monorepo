...
export const ChangeEstimatedFeeSheetContainer = () => {
    ...
    const estimatedFeeEth = React.useMemo(() => {
        const gwei = Number(maxFeeGwei)
        const gas = Number(gasLimit)
        if (!isFinite(gwei) || !isFinite(gas) || gwei <= 0 || gas <= 0) return '0'
        // fee(ETH) = gasLimit * maxFeePerGas(gwei) / 1e9
        const feeEth = (gas * gwei) / 1e9
        return feeEth.toPrecision(6)
    }, [maxFeeGwei, gasLimit])
    ...
    return (
        <SafeBottomSheet title="Adjust network fee">
            <View width="100%" gap="$3">
                ...
                >
                    ...
                    <Text fontWeight={700} fontSize="$5">{`~${estimatedFeeEth} ${nativeSymbol}`}</Text>
                </View>
