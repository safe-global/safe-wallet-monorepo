...
const getDelegateKeyId = (safe: string, delegateAddress: string): string => `delegate_${safe}_${delegateAddress}`

const getSignerFromDelegate = async (delegate: DelegateInfo) => {
...
const registerForNotificationsOnBackEnd = async ({
  safeAddress,
  signer,
  chainIds,
  fcmToken,
...
const unregisterForNotificationsOnBackEnd = async ({
  signer,
  safeAddress,
  chainIds,
}: {
...
const getAccountType = (safeAddress: string) => {
...
export async function subscribeSafe(address: string, chainIds: string[]): Promise<void> {
  try {
    Logger.info('subscribeSafe ::', { address, chainIds })

    const delegate = selectFirstDelegateForAnySafeOwner(store.getState(), address as `0x${string}`)
    const { signer } = await getSignerFromDelegate(delegate)
    const { accountType } = getAccountType(address)
    ...
    Logger.error('subscribeSafe failed', err)
  }
}

export async function unsubscribeSafe(address: string, chainIds: string[]): Promise<void> {
  try {
    Logger.info('unsubscribeSafe ::', { address, chainIds })
    ...
    Logger.error('unsubscribeSafe failed', err)
  }
}
