...
describe('signersSlice', () => {
  it('adds a signer', () => {
    const signer: AddressInfo = { value: '0x123', name: 'Alice' }
    const state = signersReducer(undefined, addSigner(signer))
    expect(state['0x123']).toEqual(signer)
  })

  describe('addSignerWithEffects', () => {
    const mockSigner: AddressInfo = {
    ...
    it('should set active signer when activeSafe exists and no active signer for that safe', async () => {
      const safeAddress = '0x1234567890123456789012345678901234567123' as `0x${string}`
    ...
    it.only('should work correctly with multiple signers and safes', async () => {
      const safeAddress1 = '0x1111111111111111111111111111111111111111' as `0x${string}`
      const safeAddress2 = '0x2222222222222222222222222222222222222222' as `0x${string}`
      ...
      const signer1: AddressInfo = {
        value: '0x1111111111111111111111111111111111111111',
        name: 'Signer 1',
      }

      const signer2: AddressInfo = {
