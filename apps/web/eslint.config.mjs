...
export default [
  ...
  ...compat.extends('next', 'prettier', 'plugin:storybook/recommended'),
  {
    ...
    rules: {
      ...
      // Design-system consistency: on a primitive, `className` is for LAYOUT only (w-full,
      // margins, grid placement). Height/padding/font-size/radius/background are owned by the
      // `size`/`variant` props — overriding them via className is what makes components drift.
      //
      // The guards live in the design system so this app, apps/web-tanstack and the package all
      // enforce one contract: packages/design-system/eslint/rules.cjs, covered by rules.test.ts.
      // Add a variant to the primitive rather than disabling a rule.
