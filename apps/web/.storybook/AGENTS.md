...
Three top-level groups (design-system atoms live in the design system's own Storybook, under `UI/`
and `Presets/`):
...
## Design-system rules live in the design system

The rules about **which** component to use and **how** to style it — variants over custom
`className`, the closed presets, the token and surface traps — are owned by
[`packages/design-system/AGENTS.md`](../../../packages/design-system/AGENTS.md), together with the
ESLint guards that enforce them (`packages/design-system/eslint/rules.cjs`, which this app's
`eslint.config.mjs` consumes).

Read that file before writing or restyling any UI. It is the single, tool-agnostic source of truth —
point Cursor `.mdc` / Copilot instructions there rather than duplicating it, because duplicated
rules drift.

The short version: on a design-system component, `className` is **layout only** (`w-*`, margins,
flex/grid). Height, padding, font size, radius, background and border belong to the `variant`/`size`
props. If no variant fits a recurring need, add one to the primitive.

Design-system primitives, presets and their stories live in `@safe-global/design-system` and are
shown in **its** Storybook (`yarn workspace @safe-global/design-system storybook`). This Storybook
covers what the app owns: `Components/`, `Features/` and `Pages/`.

The rest of this file is about the part that is genuinely app-specific — rendering an app component
in Storybook needs mocked data, a store and provider contexts, none of which the design system has.
