...
export const replayStepUpAction = async (dispatch: AppDispatch, pending: PendingStepUpAction): Promise<void> => {
  ...
  // During page load an invalidated query can still be in flight; RTK Query won't refetch it and its
  // `delayed` invalidation doesn't wait, so it can keep a pre-write response. Invalidating again once
  // nothing is in flight refetches with the written data.
