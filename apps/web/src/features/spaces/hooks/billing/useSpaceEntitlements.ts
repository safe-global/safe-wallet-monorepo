...
export const useSpaceEntitlements = (spaceId?: string | null) => {
  ...
  const { data, isLoading, isUninitialized, isError, refetch } = useEntitlementsGetEntitlementsV1Query(
    gatedSpaceId ? { spaceId: gatedSpaceId } : skipToken,
    SPACE_REFRESH_OPTIONS,
  )

  return { plan: data?.plan ?? null, seats: getSeatsMeter(data), isLoading, isUninitialized, isError, refetch }
}
