...
import { isEmailIdentifier } from './utils'
  ...
  return (
    <Field className="relative">
      ...
      <Input
        ...
        className="border-[var(--color-border-light)] bg-[var(--color-background-paper)] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)] focus-visible:border-[var(--color-border-main)]"
        {...inputProps}
        ref={(element) => {
      ...
      {showOptions && inputRect
        ? createPortal(
            <div
              className="fixed mt-1 overflow-hidden rounded-md border border-[var(--color-border-light)] bg-[var(--color-background-paper)] text-[var(--color-text-primary)] shadow-md"
              style={{
                left: inputRect.left,
            ...
            >
              {options.map((option) => (
                <button
                  className="w-full appearance-none border-0 bg-[var(--color-background-paper)] px-4 py-2 text-left text-[var(--color-text-primary)] outline-none hover:bg-[var(--color-background-secondary)] focus:bg-[var(--color-background-secondary)] focus:outline-none focus-visible:outline-none"
                  key={option.address}
