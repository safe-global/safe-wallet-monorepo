...
type QueryResult = ReturnType<typeof policiesApi.usePoliciesGetActivePoliciesV1Query>
...
describe('useActivePolicies', () => {
  ...
  beforeEach(() => {
    ...
    querySpy = jest.spyOn(policiesApi, 'usePoliciesGetActivePoliciesV1Query').mockReturnValue(mockQueryResult())
  })

  afterEach(() => jest.restoreAllMocks())

  it('queries with the space-scoped args and refresh options', () => {
    renderHook(() => useActivePolicies(CHAIN_ID, SAFE))
  ...
  it('maps the query response items to `policies`', () => {
    const items = [spendingLimitPolicyBuilder().build(), tokenWithdrawPolicyBuilder().build()]
    querySpy.mockReturnValue(mockQueryResult({ currentData: { items } }))

    const { result } = renderHook(() => useActivePolicies(CHAIN_ID, SAFE))
