...
import { limitSummaryBuilder, limitSummaryTokenBuilder } from '../testBuilders'
...
describe('LimitSummaryRow', () => {
  it('shows the amount with the token symbol and the frequency label', () => {
    ...
    expect(screen.getByTestId('policy-summary-limit')).toHaveTextContent('0.5466 ETH')
    expect(screen.getByTestId('policy-summary-frequency')).toHaveTextContent('Weekly')
  })
  ...
  it('formats the amount through the shared formatter once the decimals are known', () => {
    ...
    expect(screen.getByTestId('policy-summary-limit')).toHaveTextContent(`${formatVisualAmount('1000000000', 6)} USDC`)
  })

  it('renders the amount exactly as typed when parsing fails despite known decimals', () => {
  ...
  it('renders the amount exactly as typed when the token decimals are unknown', () => {
    ...
    expect(screen.getByTestId('policy-summary-limit')).toHaveTextContent('12.5 0x1234…5678')
    expect(screen.getByTestId('policy-summary-frequency')).toHaveTextContent('One time only')
  })
  ...
  it("falls back to the chain's dropdown wording for a test-chain period", () => {
    ...
    expect(screen.getByTestId('policy-summary-frequency')).toHaveTextContent('1 hour')
  })
})
