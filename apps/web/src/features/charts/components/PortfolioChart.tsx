...
import { usePortfolioGetWalletChartV1Query, type WalletChart } from '@safe-global/store/gateway/AUTO_GENERATED/portfolios'
...
const PortfolioChart = (): ReactElement => {
  ...
  const {
    ...
    {
      skip: !safeAddress || !currency,
    },
  )

  /** Sample value at any timestamp using binary search O(log n) + linear interpolation */
  const sampleValueAtTime = useCallback((data: typeof chartData, targetTime: number): number => {
    ...
    const [t1, v1] = points[left]
    const [t2, v2] = points[right]
  ...
  const interpolateChartData = useCallback(
    (oldData: WalletChart, newData: WalletChart, progress: number): WalletChart => {
      ...
      for (let i = 0; i < targetPointCount; i++) {
        ...
        const oldValue = sampleValueAtTime(oldData, timestamp)
        const newValue = sampleValueAtTime(newData, timestamp)
        ...
        const value = oldValue * (1 - blendWeight) + newValue * blendWeight
        interpolatedPoints.push([timestamp, value])
      }
  ...
  const drawChart = useCallback(
    (
      ...
      const points = data.points.map((point, index) => {
        ...
        const y = padding.top + chartHeight - ((point[1] - minValue) / valueRange) * chartHeight
        return { x, y, value: point[1], timestamp: point[0] }
      })

      const firstValue = data.points[0][1]
      const lastValue = data.points[data.points.length - 1][1]
  ...
  useEffect(() => {
    ...
    const values = dataToRender.points.map((p) => p[1])
    const minValue = Math.min(...values)
    const maxValue = Math.max(...values)
    ...
    ctx.textAlign = 'left'
    ctx.fillText(`$${maxValue.toFixed(2)}`, width - padding.right + 5, padding.top + 5)
    ctx.fillText(`$${minValue.toFixed(2)}`, width - padding.right + 5, height - padding.bottom)
  ...
  useEffect(() => {
    ...
    const values = chartData.points.map((p) => p[1])
    const minValue = Math.min(...values)
    const maxValue = Math.max(...values)
    const valueRange = maxValue - minValue || 1
    ...
    const y = padding.top + chartHeight - ((hoveredPoint.value - minValue) / valueRange) * chartHeight
    ...
    const firstValue = chartData.points[0][1]
    const lastValue = chartData.points[chartData.points.length - 1][1]
    const isPositive = lastValue >= firstValue
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (point) {
      ...
      setHoveredPoint({ value: point[1], time: timeStr, index: clampedIndex })
    }
  }
  ...
  const displayValue = hoveredPoint?.value ?? lastPoint[1]
  ...
  return (
    <Box>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={2}>
        <Box>
          <Typography variant="h5" fontWeight="bold">
            <FiatValue value={displayValue.toString()} />
          </Typography>
      ...
      {/* Canvas Chart */}
      <Box ref={containerRef} sx={{ position: 'relative', width: '100%', height: `${CHART_CONFIG.height}px` }}>
