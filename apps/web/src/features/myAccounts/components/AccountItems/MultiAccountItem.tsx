    ...
    >
      <Collapsible open={expanded} onOpenChange={toggleExpand}>
        <CollapsibleTrigger
          ...
          // `nativeButton={false}` required when rendering as a non-<button>: without it Base UI emits no
          // `role="button"` or Enter/Space handling, so keyboard users could focus but never expand the group.
          nativeButton={false}
