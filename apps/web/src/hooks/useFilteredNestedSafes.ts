 ...
 * - Invalid: Deployed by anyone else
 *
 * Returns all safes sorted with valid ones first, then invalid ones.
...
export function useFilteredNestedSafes(rawNestedSafes: string[], chainId: string): UseFilteredNestedSafesResult {
  ...
  const [nestedSafes, setNestedSafes] = useState<NestedSafeItem[]>([])
  ...
  useEffect(() => {
    const validateNestedSafes = async () => {
      ...
      try {
        ...
        const validatedSafes: NestedSafeItem[] = creationResults.map(({ address, creator }) => {
        ...
        // Sort: valid safes first, then invalid ones
        validatedSafes.sort((a, b) => {
          if (a.isValid === b.isValid) return 0
          return a.isValid ? -1 : 1
        })

        setNestedSafes(validatedSafes)
      } catch {
  ...
  return { nestedSafes, isLoading, startFiltering, hasStarted }
}
