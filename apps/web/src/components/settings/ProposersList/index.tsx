...
const ProposersList = () => {
  ...
  const gate = usePlanGate(FEATURES.PROPOSER_GATING)
  useMigrateProposerLabels()
  ...
  return (
    <div data-testid="proposer-section">
      ...
      {isEnabled &&
        (gate.isBlocked ? (
          <div className="mb-4">
            <SafeProLock title="Adding proposers requires Safe Pro" href={gate.upgradeHref} />
          </div>
        ) : (
          !gate.isLoading && <AddProposerButton onAdd={onAdd} isUndeployedSafe={isUndeployedSafe} />
        ))}
