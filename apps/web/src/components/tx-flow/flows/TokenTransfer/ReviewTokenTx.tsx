...
const ReviewTokenTx = ({ onSubmit, txNonce }: { onSubmit: () => void; txNonce?: number }): ReactElement => {
  ...
  return isSpendingLimitTx && data?.recipients.length === 1 ? (
    ...
    <ReviewSpendingLimitTx params={data.recipients[0]} onSubmit={onSubmit} />
  ) : (
    <ReviewTokenTransfer params={data} onSubmit={onSubmit} txNonce={txNonce} />
  )
}
