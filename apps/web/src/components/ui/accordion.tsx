  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          // cursor-pointer on the base: every trigger opens its panel, so the affordance belongs here
...
function AccordionContent({ className, children, ...props }: AccordionPrimitive.Panel.Props) {
  return (
    ...
    >
      <div
        className={cn(
          'pt-0 pb-4 [&_:where(a:hover)]:text-foreground h-(--accordion-panel-height) data-ending-style:h-0 data-starting-style:h-0 [&_a]:underline [&_a]:underline-offset-3 [&_p:not(:last-child)]:mb-4',
          className,
        )}
      >
        {children}
