...
/**
 * Two heights: `default` (h-9) mirrors SelectTrigger's default so a text input and a select on
 * the same row line up, and `hero` (66px) is the tall Safe-creation / big-filter field. Prefer
 * `inputSize` over a hand-rolled `className="h-…"`. Border uses the shared `--border` token (not a
 * hard-coded gray). NB: `--input` is #fff in light mode (invisible), so a visible field border must
 * use `border-border`, like AddressInput.
 */
