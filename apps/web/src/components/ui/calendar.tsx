 ...
 *
 * Cell geometry comes from the `--cell-size` variable, so a caller can scale the whole grid with a
 * single class (`[--cell-size:--spacing(11)]`). `captionLayout="dropdown"` swaps the month/year
 * label for selects; the nav arrows sit in the caption's reserved padding so they stay clickable.
 *
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        // `px-(--cell-size)` reserves room for the absolutely positioned nav. Keep this box out of
        // the positioned flow, or it paints over the arrows and swallows their clicks.
        month_caption: cn(
        ...
        dropdown_root: cn(
          'has-focus:border-ring has-focus:ring-ring/50 relative rounded-md border border-border shadow-xs has-focus:ring-[3px]',
