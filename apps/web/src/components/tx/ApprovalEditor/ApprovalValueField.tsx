...
export const ApprovalValueField = ({ name, tx, readOnly }: { name: string; tx: ApprovalInfo; readOnly: boolean }) => {
  ...
  return (
    <Combobox
      items={selectValues}
      // Bind `value` alongside `inputValue`: Base UI resets to the selected value on close, so an
      // uncontrolled selection wipes a typed amount and the default filter hides every preset.
      value={value ?? ''}
