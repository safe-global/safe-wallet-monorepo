...
import { Family, Row, Swatch, StoryLink, LinkGroup, WhereUsed } from './gallery-kit'
import SubmitButton from '../presets/SubmitButton'
import { ActionButton } from '../presets/ActionBar'
import IconAction from '../presets/IconAction'
import ChoiceButton from '../presets/ChoiceButton'
import SplitMenuButton from '../presets/SplitMenuButton'
import DialogActions from '../presets/DialogActions'
import OnboardingFooter from '../presets/OnboardingFooter'
...
const TokenOptionItem = ({ token }: { token: (typeof DEMO_TOKENS)[number] }) => (
  <div className="flex items-center gap-2">
    {/* The app renders its own TokenIcon here (chain-aware, with a fallback asset). The design
        system has no chain concept, so the gallery shows the same shape with a plain Avatar. */}
  ...
  render: () => (
    ...
    >
      <Row label="Actions we use" note="the real buttons, by intent — label · variant / size · and where they live">
        ...
        <Swatch label="default · submit" to="Presets/DialogActions" toLabel="DialogActions">
        ...
        <Swatch label="outline · submit" to="Presets/DialogActions" toLabel="DialogActions">
      ...
      >
        ...
        <Swatch label="submit · h-10 → SubmitButton" to="Presets/DialogActions" toLabel="DialogActions">
      ...
      <WhereUsed>
        ...
        <LinkGroup label="Reference">
          ...
          <StoryLink title="Presets/SubmitButton">SubmitButton</StoryLink>
          <StoryLink title="Presets/ActionBar">ActionBar</StoryLink>
          <StoryLink title="Presets/DialogActions">DialogActions</StoryLink>
          <StoryLink title="Presets/IconAction">IconAction</StoryLink>
        </LinkGroup>
      </WhereUsed>
    </Family>
  ),
...
export const Tables: Story = {
  render: () => (
    <Family
      ...
      lead={
        <>
          The design system ships one table: the <code>Table</code> shell below — the row/cell rhythm, header treatment
          and borders every grid inherits. The grid <em>types</em> built on it (sortable, paginated, drag-reorderable,
          mobile-card) are app components; they are catalogued in the app Storybook under{' '}
          <code>Components/Common/Tables</code>, together with the open question about whether the two generic grids
          should unify.
        </>
      }
    >
      <Row label="The shell" note="what a grid inherits before an app grid type adds sorting, paging or selection">
        <Swatch
          label="Table"
          use="Rarely rendered directly — reach for an app grid type instead. Shown here so the base rhythm is reviewable on its own."
          to="UI/Table"
          toLabel="UI/Table"
        >
          <div className="w-full max-w-2xl">
            <Table>
              ...
              <TableBody>
                {[
                  { name: 'alice.eth', role: 'Owner', threshold: '2 of 3' },
                  { name: 'bob.eth', role: 'Owner', threshold: '2 of 3' },
                  { name: 'carol.eth', role: 'Proposer', threshold: '—' },
                ].map((row) => (
      ...
      <WhereUsed>
        <LinkGroup label="Reference">
          <StoryLink title="UI/Table">Table (the shared shell)</StoryLink>
        </LinkGroup>
      </WhereUsed>
    </Family>
  ),
