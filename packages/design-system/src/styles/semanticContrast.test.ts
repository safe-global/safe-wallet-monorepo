...
describe('semantic tint contrast', () => {
  THEMES.forEach(({ name, vars, shadcn }) => {
    const varsBlock = readBlock('brand-vars.css', vars)
    const foreground = readVar(readBlock('tokens.css', shadcn), '--foreground')
  ...
  it('records why *-dark must not be used as body text on its own tint', () => {
    ...
    const light = readBlock('brand-vars.css', ':root')
    const dark = readBlock('brand-vars.css', "[data-theme='dark']")
