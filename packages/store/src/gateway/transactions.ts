...
export const txHistoryApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    // Infinite query version of the pending transactions (queue) query
    getPendingTxsInfinite: build.infiniteQuery<
      QueuedItemPage, // Page content type (entire page response)
      PendingTxsInfiniteQueryArg, // Query arg type (without cursor)
      string | null // Page param type (cursor)
    >({
      // Define infinite query options
      infiniteQueryOptions: {
        ...
        getNextPageParam: (lastPage, allPages, lastPageParam, allPageParams) => {
          ...
          // This is more robust than using string.split when dealing with complex URLs
          try {
            // The URL might be a relative URL like /v1/chains/{chainId}/safes/{safeAddress}/transactions/queued?cursor=XYZ&other=param
            // or a full URL with hostname
            const urlParts = lastPage.next.split('?')
            if (urlParts.length < 2) {
              return undefined // No query string in the URL
...
// Export the generated hooks directly
