 ...
 *
 * Consensus / coordinator / oracle addresses default to the checked-in devnet
 * golden vector but can be overridden with SAFENET_CONSENSUS / SAFENET_COORDINATOR
 * / SAFENET_ORACLE (e.g. the SENTINEL_ORACLE_V2 emitter for log correlation).
...
const makeReader = () =>
  new SafenetReader({
    ...
    oracle: process.env.SAFENET_ORACLE ?? golden.oracle,
  })
