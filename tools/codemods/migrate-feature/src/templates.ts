...
export function generateContractTemplate(config: FeatureConfig): string {
  ...
  lines.push(` * - useSomething → hook (stub returns {})`)
  lines.push(` * - camelCase → service (stub is no-op)`)
  lines.push(` */`)
  lines.push(``)
  ...
  // Type imports for hooks
  if (publicAPI.hooks.length > 0) {
    lines.push(`// Hook imports`)
    for (const hook of publicAPI.hooks) {
      lines.push(`import type { ${hook} } from './hooks/${hook}'`)
    }
    lines.push(``)
  }
  ...
  lines.push(`/**`)
  lines.push(` * ${featureNamePascal} Feature Implementation - flat structure`)
  ...
  if (publicAPI.components.length > 0) {
    ...
    if (publicAPI.hooks.length > 0 || publicAPI.services.length > 0) {
      lines.push(``)
    }
  }
  ...
  // Hooks
  if (publicAPI.hooks.length > 0) {
    lines.push(`  // Hooks (useSomething) - stub returns {}`)
    for (const hook of publicAPI.hooks) {
      lines.push(`  ${hook}: typeof ${hook}`)
    }
    if (publicAPI.services.length > 0) {
      lines.push(``)
    }
  ...
  // Services
  if (publicAPI.services.length > 0) {
    lines.push(`  // Services (camelCase) - stub is no-op`)
    for (const service of publicAPI.services) {
...
export function generateFeatureTemplate(config: FeatureConfig): string {
  ...
  // Direct imports for hooks
  if (publicAPI.hooks.length > 0) {
    lines.push(`// Hook imports`)
    for (const hook of publicAPI.hooks) {
      lines.push(`import { ${hook} } from './hooks/${hook}'`)
    }
    lines.push(``)
  }
...
export function generateIndexTemplate(config: FeatureConfig): string {
  ...
  lines.push(` * import { ${featureNamePascal}Feature } from '@/features/${featureName}'`)
  ...
  lines.push(` * \`\`\``)
  lines.push(` *`)
  lines.push(` * All feature functionality is accessed via flat structure from useLoadFeature().`)
  lines.push(` * Naming conventions determine stub behavior:`)
  lines.push(` * - PascalCase → component (stub renders null)`)
  lines.push(` * - useSomething → hook (stub returns {})`)
  lines.push(` * - camelCase → service (stub is no-op)`)
